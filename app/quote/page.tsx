...
function getItemWeightKg(item: QuoteItem) {
  const maybeWeight = (item as QuoteItem & { weight?: number }).weight;

  if (typeof maybeWeight === "number" && Number.isFinite(maybeWeight) && maybeWeight > 0) {
    return maybeWeight;
  }
...
export default function QuotePage() {
  ...
  const selectedItems = useMemo(
    () => items.filter((item) => selectedKeySet.has(getVariantKey(item.productId, item.variation))),
  ...
  const cddCharge = useMemo(() => selectedQuantity * CDD_RATE_PER_UNIT, [selectedQuantity]);
  const payNow = productPrice + cddCharge;
  const estimatedShipping = useMemo(() => {
    return productGroups.reduce((groupTotal, group) => {
      ...
      const totalWeight = selectedGroupItems.reduce(
        (sum, item) => sum + getItemWeightKg(item) * item.quantity,
        0,
      );

      return groupTotal + totalWeight * selectedProfile.ratePerKg;
    }, 0);
  ...
  const handlePlaceOrder = () => {
    ...
    const order: StoredOrder = {
      ...
      summary: {
        ...
        cddCharge,
        payNow,
        payOnDelivery: estimatedShipping,
      },
    };
    ...
    setSelectedKeys((current) =>
      current.filter(
        (key) =>
          !selectedItems.some(
            (item) => getVariantKey(item.productId, item.variation) === key,
          ),
      ),
    );
    setItems(getQuoteItems());
          ...
          <div className="grid gap-6 lg:grid-cols-[minmax(0,1fr)_340px] lg:items-start">
            <div className="space-y-4">
              ...
              {productGroups.map((group) => {
                ...
                const groupShipping = selectedGroupItems.reduce(
                  (sum, item) => sum + getItemWeightKg(item) * item.quantity * selectedProfile.ratePerKg,
                  0,
                );
                              ...
                              <div className="flex gap-3">
                                ...
                                <div className="min-w-0 space-y-1">
                                  ...
                                  <p className="text-xs text-slate-400">
                                    Weight basis: {getItemWeightKg(item)} kg per unit
                                  </p>
                                </div>
                              </div>
                      ...
                      <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
                        <div className="space-y-2 lg:max-w-md">
                          ...
                          <div className="flex flex-col gap-1 text-xs text-slate-500 sm:flex-row sm:items-center sm:justify-between">
                            ...
                            <span>Group shipping: {formatBDT(groupShipping)}</span>
                          </div>
            ...
            <aside className="space-y-5 lg:sticky lg:top-6">
              ...
              <div className="space-y-0 rounded-xl border border-slate-200 bg-white px-5">
                <SummaryRow label="Quantity" value={String(selectedQuantity)} />
                <SummaryRow label="Product Price" value={formatBDT(productPrice)} />
                <SummaryRow label="CDD Charge" value={formatBDT(cddCharge)} />
                <SummaryRow label="Pay Now" value={formatBDT(payNow)} strong />
              </div>
