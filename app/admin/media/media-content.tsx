...
export default function MediaContent() {
  ...
  const filteredItems = useMemo(() => {
    ...
    return items.filter((item) => {
      ...
      const ageInDays = Number.isNaN(timestamp) ? Number.POSITIVE_INFINITY : (now - timestamp) / (1000 * 60 * 60 * 24);
      const matchesDate =
        dateFilter === "all" ||
        ...
        <div className="flex flex-col gap-4 border-b border-gray-200 px-6 py-4 lg:flex-row lg:items-center lg:justify-between">
          <div className="flex flex-col gap-3 sm:flex-row sm:items-center">
            <select
              value="all-media"
              disabled
              className="h-11 rounded-lg border border-gray-300 bg-white px-4 text-sm font-medium text-slate-700 outline-none disabled:cursor-default disabled:opacity-100"
            >
          ...
          <div className="grid min-h-[620px] grid-cols-1 xl:grid-cols-[minmax(0,1fr)_320px]">
              ...
              <div className="flex h-full items-center justify-center p-6">
                ...
                >
                  ...
                  <p className="mt-2 text-sm text-slate-400">
                    Recommended image ratio depends on your product gallery use case.
                  </p>
                  {isUploading ? (
                    <span className="mt-6 inline-flex rounded-full bg-[#615FFF]/10 px-4 py-2 text-sm font-semibold text-[#615FFF]">
                      Uploading...
                    </span>
                  ) : null}
                </button>
            ...
            <aside className="bg-white">
              <div className="border-b border-gray-200 px-6 py-6">
                <h4 className="text-lg font-semibold uppercase tracking-[0.12em] text-slate-700">
                  Attachment Details
                </h4>
              </div>

              <div className="p-6">
                <div className="rounded-2xl border border-dashed border-slate-200 px-6 py-8 text-center text-sm leading-6 text-slate-500">
                  ...
                  <div className="space-y-5">
                    <div className="overflow-hidden rounded-2xl border border-slate-200 bg-slate-50">
                      <div
                        ...
                        style={{ backgroundImage: `url("${selectedItem.publicUrl}")` }}
                      />
                    </div>
                    ...
                    <div className="space-y-3">
                      {selectMode ? (
                        <button
                          ...
                          className="inline-flex w-full items-center justify-center rounded-xl bg-[#615FFF] px-4 py-3 text-sm font-semibold text-white transition-opacity hover:opacity-90"
                        >
                          Use This Image
                        </button>
                      ) : null}
                      <button
                        type="button"
                        onClick={() => void handleCopy(selectedItem.publicUrl)}
                        className="inline-flex w-full items-center justify-center rounded-xl border border-slate-300 bg-white px-4 py-3 text-sm font-medium text-slate-700 transition-colors hover:border-[#615FFF]/40 hover:text-slate-900"
                      >
                        Copy URL
                      </button>
                      <a
